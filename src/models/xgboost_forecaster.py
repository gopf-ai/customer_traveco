...
class XGBoostForecaster(BaseForecaster):
    ...
    def fit(
        ...
        logger.info(f"Training samples: {len(df_clean)} (removed {len(df_features) - len(df_clean)} with missing features)")
        ...
        self.training_metrics = self._calculate_metrics(y_train.values, train_pred)

        logger.info(f"✅ Training complete")
        logger.info(f"  Training MAPE: {self.training_metrics['mape']:.2f}%")
