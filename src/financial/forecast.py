"""Financial forecast generation pipeline
...
class FinancialForecast:
    """
    Generate financial forecasts using best model per metric
    ...
    def run(self) -> pd.DataFrame:
        ...
        """
        logger.info("=" * 60)
        logger.info("FINANCIAL FORECAST GENERATION")
        logger.info("=" * 60)
        ...
        # Step 5: Save output
        output_file = self.output_path / 'financial_forecasts.csv'
        df_combined.to_csv(output_file, index=False)
        logger.info(f"\nSaved financial forecasts to: {output_file}")
    ...
    def _load_eval_results(self) -> pd.DataFrame:
        ...
        input_path = self.intermediate_path / 'financial_model_eval.csv'
