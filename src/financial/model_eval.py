"""Financial model evaluation pipeline

Trains and evaluates all models on financial metrics using train/test split.
"""
...
# Metrics that benefit from working_days (based on correlation analysis)
# Financial metrics show negative correlation with working days
...
class FinancialModelEval:
    """
    Evaluate forecasting models on financial metrics
    ...
    def run(self) -> pd.DataFrame:
        ...
        """
        logger.info("=" * 60)
        logger.info("FINANCIAL MODEL EVALUATION")
        logger.info("=" * 60)
        ...
        # Step 5: Save results
        output_path = self.intermediate_path / 'financial_model_eval.csv'
    ...
    def _load_time_series(self) -> pd.DataFrame:
        """Load prepared financial time series"""
        input_path = self.intermediate_path / 'financial_time_series.csv'

        if not input_path.exists():
            raise FileNotFoundError(
                f"Time series file not found: {input_path}\n"
                f"Run financial data_prep.py first"
            )
        ...
        logger.info(f"  Metrics available: {[col for col in df.columns if col not in ['date', 'year', 'month', 'working_days']]}")
...
def run():
    ...
    eval_pipeline = FinancialModelEval()
